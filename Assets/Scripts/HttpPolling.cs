...
public class TssHttpPolling : MonoBehaviour
{
    // Reference your UI Text components
    ...
    {
        // Update UI elements with telemetry data
        if (latestData.highFrequency != null)
        {
            // Update oxygen display - using eva1_oxy
            if (oxygenText != null)
                oxygenText.text = $"{latestData.highFrequency.eva1_oxy:F1}%";
                
            // Update battery display - using eva1_batt
            if (batteryText != null)
                batteryText.text = $"{latestData.highFrequency.eva1_batt:F1}%";
        }
        
        if (latestData.lowFrequency != null)
        {
            // Update BPM display - using eva1_heart_rate
            if (bpmText != null)
                bpmText.text = $"{latestData.lowFrequency.eva1_heart_rate:F0}";
                
            // Update temperature display - using eva1_temperature
            if (temperatureText != null)
                temperatureText.text = $"{latestData.lowFrequency.eva1_temperature:F1}°C";
        }
        
        // Your existing debug logs can stay for testing
    }
}