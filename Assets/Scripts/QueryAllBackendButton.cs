...
public class TssDataFetcher : MonoBehaviour
{
    // IMPORTANT: Replace this with the HTTPS URL Ngrok gave you!
    public string ngrokUrl = "https://b36e-128-195-95-16.ngrok-free.app";

    // --- Data Structures (Mirroring Backend JSON) ---
    // You might need to adjust these based on the exact fields
    // you have in HighFrequencyData and LowFrequencyData in types.ts
    [System.Serializable]
    public class TelemetryData // Base class common fields
    {
    ...
    void OnMouseDown()
    {
        Debug.Log("Object Clicked! Fetching TSS data...");
        StartCoroutine(FetchTssData());
    }

    // --- Coroutine for Web Request ---
    ...
    void ParseTssData(string jsonData)
    {
        try
        {
            ...
            if (response != null && response.success)
            {
                ...
                // Now you can use the data in response.highFrequency and response.lowFrequency
                // to update your Unity game state, UI, etc.
